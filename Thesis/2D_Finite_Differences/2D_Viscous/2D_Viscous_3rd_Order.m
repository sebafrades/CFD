    ...
    % Se calculan las líneas de corriente interiores
        ...
        for i=fh+1:N-1 % fila 5 a 7
            for j=2:cquiebre % columna 2 a 11
                ...
                %Aneu(i,j)=(-Bneu(i,j)*deltax^2-(Aneu(i+1,j)+Aneu(i-1,j)+Aneu(i,j-1)+Aneu(i,j+1)))/(-4);
            end
        end
	...
	% Vorticidad
        ...
        for i=fh+1:N-1 % fila 5 a 7
            for j=2:cquiebre % columna 2 a 11
                ...
                u=(Aneu(i-1,j)-Aneu(i+1,j))/(2*deltax);
                ...
                v=(Aneu(i,j+1)-Aneu(i,j-1))/(2*deltax);
                ...
                w_x=(Bneu(i,j+1)-Bneu(i,j-1))/(2*deltax);
                w_y=(Bneu(i-1,j)-Bneu(i+1,j))/(2*deltax);
                
                if i==fh+1
                    Bneu(i,j)=(u*w_x+v*w_y-(1/Re)*(((Bneu(i,j+1)+Bneu(i,j-1))/deltax^2)+((Bneu(i-1,j)+Bneu(i+1,j))/deltax^2)))/((1/Re)*(-4/deltax^2));
                elseif i==N-1
                    Bneu(i,j)=(u*w_x+v*w_y-(1/Re)*(((Bneu(i,j+1)+Bneu(i,j-1))/deltax^2)+((Bneu(i-1,j)+Bneu(i+1,j))/deltax^2)))/((1/Re)*(-4/deltax^2));
                elseif j==2
                    Bneu(i,j)=(u*w_x+v*w_y-(1/Re)*(((Bneu(i,j+1)+Bneu(i,j-1))/deltax^2)+((Bneu(i-1,j)+Bneu(i+1,j))/deltax^2)))/((1/Re)*(-4/deltax^2));
                elseif j==cquiebre
                    ...
                    w_x_minus=(Bneu(i,j-2)-3*Bneu(i,j-1)-Bneu(i,j+1))/(3*deltax);
                    w_x_plus=(Bneu(i,j-1)+3*Bneu(i,j+1)-Bneu(i,j+2))/(3*deltax);
                    w_y_minus=(Bneu(i+2,j)-3*Bneu(i+1,j)-Bneu(i-1,j))/(3*deltax);
                    w_y_plus=(Bneu(i+1,j)+3*Bneu(i-1,j)-Bneu(i-2,j))/(3*deltax);
                    
                    division=-q*u_plus/deltax+q*u_minus/deltax-q*v_plus/deltax+q*v_plus/deltax+(1/Re)*(-4/deltax^2);
                    Bneu(i,j)=(u*w_x+q*(u_plus*w_x_minus+u_minus*w_x_plus)+v*w_y+q*(v_plus*w_y_minus+v_minus*w_y_plus)-(1/Re)*(((Bneu(i,j+1)+Bneu(i,j-1))/deltax^2)+((Bneu(i-1,j)+Bneu(i+1,j))/deltax^2)))/division;
                end
            end
        end
...
%fh=((1-1/R2)*(N-1)+1); % fila del valor de h
...
title (['Distribución presión, N = ',num2str(N), ', L1/h = ',num2str(R1),', L2/h = ',num2str(R3), ', H/h = ', num2str(R2), ', Re = ', num2str(Re)])
