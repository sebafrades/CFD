...
double algebraicFunction(double i) {
    double K = 1.0864;
    int L = 17;
    return (std::pow(K,i-1) - 1.0)/(std::pow(K,L-1) - 1.0);
}
...
void iterateMatrix(Matrix& x, Matrix& y, double error) {
    ...
    Matrix Phi(rows, std::vector<double>(cols, 0.0));
    Matrix Psi(rows, std::vector<double>(cols, 0.0));
    ...
    while (residual > error){
        ...
        for (int i = 1; i < rows-1; i++){
            for (int j = 1; j < cols-1; j++){
                ...
                residual = std::max(residual, std::fabs(x_new - x[i][j]));
